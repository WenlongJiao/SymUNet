...
class BaseModel():
    ...
    def update_learning_rate(self, current_iter, warmup_iter=-1):
        ...
        Args:
            current_iter (int): Current iteration or epoch (depending on training mode).
            warmup_iter (int)： Warmup iter numbers. -1 for no warmup.
                Default： -1.
        """
        # Check training mode
        update_schedule_by = self.opt['train'].get('update_schedule_by', 'iter').lower()
        is_progressive = 'iters' in self.opt['datasets']['train']
        ...
        # set up warm-up learning rate (only for iteration-based mode)
        if update_schedule_by != 'epoch' and current_iter < warmup_iter:
    ...
    def save_network(self, net, net_label, current_iter, param_key='params'):
        ...
        Args:
            ...
            current_iter (int): Current iter number or epoch number.
    ...
    def load_network(self, net, load_path, strict=True, param_key='params', remove_prefix=None):
        """Load network.
        ...
        if not os.path.exists(load_path):
            logger.warning(f"Weight file not found: {load_path}. Skipping loading.")
            return False
        ...
        if param_key is not None:
            if param_key in load_net:
                ...
                logger.info(f"Loading weights from key '{param_key}' in the .pth file.")
            else:
                pass
            
        if remove_prefix:
            logger.info(f"Removing custom prefix '{remove_prefix}' from state_dict keys.")
            load_net = {
    ...
    def save_training_state(self, epoch, current_iter):
        ...
        Args:
            ...
            current_iter (int): Current iteration (or epoch number in epoch mode).
        """
        if current_iter != -1:
            # Check if we're in epoch-based training mode
            update_schedule_by = self.opt['train'].get('update_schedule_by', 'iter').lower()
            is_progressive = 'iters' in self.opt['datasets']['train']
            
            if update_schedule_by == 'epoch' and not is_progressive:
                # Epoch mode: save epoch number as current_iter for consistency
                state = {
                    'epoch': epoch,
                    'training_mode': 'epoch',
                    'optimizers': [],
                ...
                save_filename = f'{current_iter}.state'  # Use current_iter (epoch number) as filename
            else:
                # Iter mode: save both epoch and iter
                state = {
                    'epoch': epoch,
                    'iter': current_iter,
                    'training_mode': 'iter',
    ...
    def resume_training(self, resume_state):
        ...
        """
        # Always load optimizers and schedulers regardless of training mode
        ...
        # Log training mode for information
        training_mode = resume_state.get('training_mode', 'iter')
        if training_mode == 'epoch':
            logger.info("Resuming in epoch mode: optimizers and schedulers loaded.")
        else:
            logger.info("Resuming in iteration mode: optimizers and schedulers loaded.")
