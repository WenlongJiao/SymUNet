...
def check_resume(opt, resume_iter):
    ...
    Args:
        opt (dict): Options.
        resume_iter (int): Resume iteration or epoch number.
    """
        ...
        for network in networks:
            ...
            if opt['path'].get('ignore_resume_networks') is None or (
                ...
                if update_schedule_by == 'epoch' and not is_progressive:
                    # Epoch mode: use epoch number for model filename
                    opt['path'][name] = osp.join(
                        opt['path']['models'], f'net_{basename}_{resume_iter}.pth')
                    logger.info(f"Set {name} to {opt['path'][name]} (epoch mode)")
                else:
                    # Iter mode: use iteration number for model filename
                    opt['path'][name] = osp.join(
                        opt['path']['models'], f'net_{basename}_{resume_iter}.pth')
                    logger.info(f"Set {name} to {opt['path'][name]} (iteration mode)")
